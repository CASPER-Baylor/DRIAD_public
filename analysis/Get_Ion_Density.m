function [grid, density] = Get_Ion_Density( dir_name, run_name )
...
grid_data = grid_data( (params.NUM_GRID_PTS+1):end,1);
grid_data = reshape( grid_data, [], params.RESZ, params.RESX );


for i = 1:size(grid_data,1)
   tmp = Scaler2DGrid;
   tmp = tmp.Set_Values( squeeze( grid_data(i,:,:) ) );
   density_array(i,1) = tmp;
end

density = LinearTimeSeries;
density = density.Set_Values(density_array, 1);
...
tmp_grid_data = grid_data( (params.NUM_GRID_PTS+1):end,1);
