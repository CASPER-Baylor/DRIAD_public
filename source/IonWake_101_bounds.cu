 ...
 *
 */
__global__ void checkIonDustBounds_101(float4 *const d_posIon, int *d_boundsIon,
                                       float *const d_RAD_DUST_SQRD, int *const d_NUM_DUST,
 ...
 *
 */
__global__ void injectIonSphere_101(float4 *d_posIon, float4 *d_velIon, float4 *d_accIon,
                                    curandState_t *const randStates, float *const d_RAD_SIM,
                                    int *const d_boundsIon, float *const d_GCOM,
                                    float *const d_QCOM, float *const d_VCOM,
                                    int *const d_NUM_DIV_QTH, int *const d_NUM_DIV_VEL,
                                    float *const d_SOUND_SPEED, float *const d_TEMP_ION,
                                    float *const d_PI, float *const d_TEMP_ELC, float *const d_MACH,
                                    float *const d_MASS_SINGLE_ION, float *const d_BOLTZMANN,
                                    float *const d_CHARGE_ION, int xac)
{
    ...
    if (d_boundsIon[IDion] != 0)
    {
        ...
        float randNum, floatQIndex, partQIndex, upperFloatGIndex, lowerFloatGIndex, radVel,
            part_radVel, phi, cosPhi, sinPhi, sinTheta, thetaVel, phiVel, cosTheta;
        ...
        lowerFloatGIndex = invertFind_101(&d_GCOM[tempIndex], *d_NUM_DIV_VEL, randNum);

        tempIndex = static_cast<int>(floatQIndex + 1) * *d_NUM_DIV_VEL;
        upperFloatGIndex = invertFind_101(&d_GCOM[tempIndex], *d_NUM_DIV_VEL, randNum);
        ...
        radVel = (partQIndex * upperFloatGIndex) + (1.0 - partQIndex) * lowerFloatGIndex;
        ...
        radVel = part_radVel * d_VCOM[tempIndex1] + (1.0 - part_radVel) * d_VCOM[tempIndex];
        ...
        d_velIon[IDion].y =
            -((radVel * sinTheta + thetaVel * cosTheta) * sinPhi + phiVel * cosPhi) * velScale;
        d_velIon[IDion].x =
            -((radVel * sinTheta + thetaVel * cosTheta) * cosPhi - phiVel * sinPhi) * velScale;
...
__global__ void injectIonCylinder_101(
    float4 *d_posIon, float4 *d_velIon, float4 *d_accIon, curandState_t *const randStates,
    float *const d_RAD_CYL, float *const d_HT_CYL, int *const d_boundsIon, float *const d_GCOM,
    float *const d_QCOM, float *const d_VCOM, int *const d_NUM_DIV_QTH, int *const d_NUM_DIV_VEL,
    float *const d_SOUND_SPEED, float *const d_TEMP_ION, float *const d_PI, float *const d_TEMP_ELC,
    float *const d_MACH, float *const d_MASS_SINGLE_ION, float *const d_BOLTZMANN,
    float *const d_CHARGE_ION, int plasma_counter, float counter_part, int xac)
{
    ...
    if (d_boundsIon[IDion] != 0)
    {
        ...
        float randNum, lowerFloatGIndex, upperFloatGIndex, radVel, part_radVel, phi, cosPhi, sinPhi,
            sinTheta, thetaVel, phiVel, cosTheta;

        int QIndex, tempIndex, tempIndex1;
        ...
        lowerFloatGIndex = invertFind_101(&d_GCOM[pagevq + tempIndex], *d_NUM_DIV_VEL, randNum);

        upperFloatGIndex = invertFind_101(&d_GCOM[pageup + tempIndex], *d_NUM_DIV_VEL, randNum);

        // interpolate between upperFloatGIndex and lowerFloatGIndex to get
        // a normalized radial velocity that ranges from 0 to d_NUM_DIV_VEL
        radVel = (counter_part * upperFloatGIndex) + (1 - counter_part) * lowerFloatGIndex;
...
void initInjectIonSphere_101(const int NUM_DIV_QTH, const int NUM_DIV_VEL, const float TEMP_ELC,
                             const float TEMP_ION, const float DRIFT_VEL_ION, const float MACH,
                             const float MASS_SINGLE_ION, const float BOLTZMANN, const float PI,
                             float *d_QCOM, float *d_VCOM, float *d_GCOM, const bool debugMode,
                             std::ostream &fileName)
{
    ...
    for (int i = 0; i < NUM_DIV_QTH; i++)
    {
        ...
        dqn = (const1 * exp((-0.5) * vdr * vdr)) + ((0.5) * vdr * erfcf((-1.0) * const2 * vdr));
        ...
        for (int j = 0; j < NUM_DIV_VEL; j++)
        {
            ...
            Gcom[i * NUM_DIV_VEL + j] =
                (dqn - const1 * exp(-0.5 * temp * temp) - 0.5 * vdr * erfcf(const2 * temp)) / dqn;
        }
    ...
    cudaStatus =
        cudaMemcpy(d_GCOM, Gcom, sizeof(float) * NUM_DIV_QTH * NUM_DIV_VEL, cudaMemcpyHostToDevice);
    ...
    cudaStatus = cudaMemcpy(d_VCOM, Vcom, sizeof(float) * NUM_DIV_VEL, cudaMemcpyHostToDevice);
    if (cudaStatus != cudaSuccess)
    {
    ...
    cudaStatus = cudaMemcpy(d_QCOM, Qcom, sizeof(float) * NUM_DIV_QTH, cudaMemcpyHostToDevice);
    if (cudaStatus != cudaSuccess)
    {
...
void initInjectIonCylinder_101(const int NUM_DIV_QTH, const int NUM_DIV_VEL, const int TIME_EVOL,
                               const float RAD_CYL, const float HT_CYL, float *evolTe,
                               float *evolTi, float *evolVz, float *evolMach,
                               const float MASS_SINGLE_ION, const float BOLTZMANN, const float PI,
                               float *d_QCOM, float *d_VCOM, float *d_GCOM, const bool debugMode,
                               std::ostream &fileName)
{
        ...
        for (int i = 0; i < NUM_DIV_QTH; i++)
        {
            ...
            for (int j = 0; j < NUM_DIV_VEL; j++)
            {
                ...
                Gcom[pagenum + i * NUM_DIV_VEL + j] =
                    (dqn - const1 * exp(-0.5 * temp * temp) - 0.5 * vdr * erfcf(const2 * temp)) /
                    dqn;
    ...
    // copy Gcom to the device
    cudaStatus = cudaMemcpy(d_GCOM, Gcom, sizeof(float) * NUM_DIV_QTH * NUM_DIV_VEL * pages,
    ...
    // copy Vcom to the device
    cudaStatus =
        cudaMemcpy(d_VCOM, Vcom, sizeof(float) * NUM_DIV_VEL * pages, cudaMemcpyHostToDevice);
    if (cudaStatus != cudaSuccess)
    {
        fprintf(stderr, "cudaMemcpy failed: d_VCOM\n");
    }
    ...
    // copy Qcom to the device
    cudaStatus =
        cudaMemcpy(d_QCOM, Qcom, sizeof(float) * NUM_DIV_QTH * pages, cudaMemcpyHostToDevice);
    if (cudaStatus != cudaSuccess)
    {
        fprintf(stderr, "cudaMemcpy failed: d_QCOM\n");
    }
...
__device__ float invertFind_101(float *const mat, int sizeMat, float y)
{
    ...
    // that y fell between
    float floatIndex = intIndex + (y - mat[intIndex - 1]) / (mat[intIndex] - mat[intIndex - 1]);
...
__global__ void boundaryEField_101(float2 *d_GRID_POS, float4 *d_GCYL_POS, int *const d_NUM_CYL_PTS,
                                   int *const d_NUM_GRID_PTS2, float *const d_INV_DEBYE,
                                   float *const d_TABLE_POTENTIAL_MULT, float *d_Vout,
                                   int plasma_counter)
{
 ...
 */
__device__ float tile_calculation_101(float2 myPosition, float invdebye, float nq, float V)
{
 ...
 */
__device__ float pointPointPotential_101(float2 bi, float4 bj, float invdebye, float ni, float vi)
{
