...
int main(int argc, char *argv[])
{
    ...
    ************************/
    ...
    fileName = inputDirName + "_params.txt";
    ...
    fileName = inputDirName + "_dust_params.txt";
    ...
    fileName = inputDirName + "_timestep.txt";
    ...
    const int NUM_ION =
        static_cast<int>(getParam_106<int>(paramFile, "NUM_ION") / (2 * DIM_BLOCK)) *
        (2 * DIM_BLOCK);
    float DEN_FAR_PLASMA = getParam_106<float>(paramFile, "DEN_FAR_PLASMA");
    float DEN_IONS = getParam_106<float>(paramFile, "DEN_IONS");
    ...
    const float MASS_SINGLE_ION = getParam_106<float>(paramFile, "MASS_SINGLE_ION");
    ...
    const float RAD_SPH_DEBYE = getParam_106<float>(paramFile, "RAD_SPH_DEBYE");
    const int NUM_DIV_VEL = getParam_106<int>(paramFile, "NUM_DIV_VEL");
    const int NUM_DIV_QTH = getParam_106<int>(paramFile, "NUM_DIV_QTH");
    const float RAD_CYL_DEBYE = getParam_106<float>(paramFile, "RAD_CYL_DEBYE");
    ...
    const int N_IONDT_PER_DUSTDT = getParam_106<int>(paramFile, "N_IONDT_PER_DUSTDT");
    const int N_IONDT_PER_PLASMADT = getParam_106<int>(paramFile, "N_IONDT_PER_PLASMADT");
    const int N_PRINT_DEN_POT = getParam_106<int>(paramFile, "N_PRINT_DEN_POT");
    ...
    const int RESX = getParam_106<int>(paramFile, "RESX");

    // Set size of the simulation region

    // debye length (m)
    float DEBYE =
        sqrt((PERM_FREE_SPACE * BOLTZMANN * TEMP_ELC) / (DEN_IONS * CHARGE_ELC * CHARGE_ELC));
    ...
    if (TIME_EVOL > 0)
    {
        ...
        if (plasma_cond_file)
        { // read in evolving plasma params from file
        ...
        if (debugMode)
        {
            debugFile << "-- First and last plasma params from file --" << '\n'
                      << evolEz[0] << ", " << evolEr[0] << ", " << evolTe[0] << ", " << evolTi[0]
                      << ", " << evolne[0] << ", " << evolni[0] << ", " << evolVz[0] << ", "
                      << evolMach[0] << '\n'
    ...
    float DEBYE_I =
        sqrt((PERM_FREE_SPACE * BOLTZMANN * TEMP_ION) / (DEN_IONS * CHARGE_ELC * CHARGE_ELC));

    // dust particle mass assumes spherical particle (Kg)
    const float MASS_DUST = DEN_DUST * (4.0 / 3.0) * PI * RAD_DUST * RAD_DUST * RAD_DUST;
    ...
    float EXTERN_ELC_MULT = ((RAD_SPH / DEBYE) + 1.0) * exp(-RAD_SPH / DEBYE) *
                            (CHARGE_SINGLE_ION * DEN_IONS * DEBYE) * (Q_DIV_M) / (PERM_FREE_SPACE);
    ...
    float vs_sq = 8 * BOLTZMANN * TEMP_ION / PI / MASS_SINGLE_ION + DRIFT_VEL_ION * DRIFT_VEL_ION;
    float RAD_COLL_MULT = 2 * Q_DIV_M * COULOMB_CONST / RAD_DUST / vs_sq;

    // the electron current to an uncharged dust grain
    float ELC_CURRENT_0 = 4.0 * PI * RAD_DUST_SQRD * DEN_FAR_PLASMA * CHARGE_ELC *
                          sqrt((BOLTZMANN * TEMP_ELC) / (2.0 * PI * ELC_MASS));
    ...
    setIonCrossSection_105(GAS_TYPE, I_CS_RANGES, NUM_DEN_GAS, MASS_SINGLE_ION, sigma_i1, sigma_i2,
                           sigma_i_tot, totIonCollFreq, debugMode, debugSpecificFile);
    ...
    if (debugMode)
    {
        ...
        for (int i = 0; i < 10; i++)
            debugFile << sigma_i_tot[i] << " ";
        debugFile << "\nFirst 10 sigma_i1" << '\n';
        for (int i = 0; i < 10; i++)
            debugFile << sigma_i1[i] << " ";
        debugFile << "\nFirst 10 sigma_i2" << '\n';
        for (int i = 0; i < 10; i++)
            debugFile << sigma_i2[i] << " ";
        debugFile << '\n';
    ...
    paramOutFile << std::setw(14) << NUM_ION << " % NUM_ION" << '\n'
    ...
    if (GEOMETRY == 0)
    {
        ...
        for (int i = 0; i < NUM_DUST; i++)
        {
            ...
            float tempDist = posDust[i].x * posDust[i].x + posDust[i].y * posDust[i].y +
    ...
    if (debugMode)
    {
        ...
        {
            debugFile << "X: " << posDust[i].x << " Y: " << posDust[i].y << " Z: " << posDust[i].z
                      << " Q: " << posDust[i].w << " VX: " << velDust[i].x << " VY " << velDust[i].y
    ...
    if (init_ions_from_file)
    { // initialize ion data from file
    ...
    { // as no initial ion data was given, initialize ion data with
    ...
    if (debugMode)
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            debugFile << "X: " << posIon[i].x << " Y: " << posIon[i].y << " Z: " << posIon[i].z
                      << std::endl;
        }
        ...
        {
            int ID = NUM_ION - i;
            debugFile << "X: " << posIon[ID].x << " Y: " << posIon[ID].y << " Z: " << posIon[ID].z
                      << std::endl;
        }
        ...
        {
            int ID = NUM_ION - i;
            debugFile << "X: " << velIon[ID].x << " Y: " << velIon[ID].y << " Z: " << velIon[ID].z
                      << std::endl;
        }
    ...
    if (TIME_EVOL > 0)
    {
        ...
        vs_sq = 8 * BOLTZMANN * TEMP_ION / PI / MASS_SINGLE_ION + DRIFT_VEL_ION * DRIFT_VEL_ION;
        RAD_COLL_MULT = 2 * Q_DIV_M * COULOMB_CONST / RAD_DUST / vs_sq;
        ELC_CURRENT_0 = 4.0 * PI * RAD_DUST_SQRD * evolne[plasma_counter] * CHARGE_ELC *
                        sqrt((BOLTZMANN * TEMP_ELC) / (2.0 * PI * ELC_MASS));
    ...
    for (int p = 0; p < num_pts; p++)
    {
        ...
        DEBYE =
            sqrt((PERM_FREE_SPACE * BOLTZMANN * evolTe[p]) / (evolne[p] * CHARGE_ELC * CHARGE_ELC));
        INV_DEBYE = 1.0 / DEBYE;
        ...
        boundaryEField_101<<<blocksPerTable, DIM_BLOCK2, sizeof(float4) * DIM_BLOCK2>>>(
            d_GRID_POS.getDevPtr(), d_GCYL_POS.getDevPtr(), d_NUM_CYL_PTS.getDevPtr(),
            d_NUM_GRID_PTS2.getDevPtr(), d_INV_DEBYE.getDevPtr(),
            d_TABLE_POTENTIAL_MULT.getDevPtr(), d_Vout.getDevPtr(), p);

        roadBlock_104(statusFile, __LINE__, __FILE__, "boundaryEField_101", print);
    }

    // Reset the DEBYE length to first condition --
    DEBYE = sqrt((PERM_FREE_SPACE * BOLTZMANN * evolTe[0]) / (evolne[0] * CHARGE_ELC * CHARGE_ELC));
    INV_DEBYE = 1.0 / DEBYE;
    d_INV_DEBYE.hostToDev();
    ...
    for (int p = 0; p < num_pts; p++)
    {
        for (int j = 0; j < NUM_GRID_PTS2; j++)
        {
            column = floor(j / NUMR);
            row = j - (column * NUMR);
            back_index = NUM_GRID_PTS2 - NUMR * (column + 1) + row;
    ...
    for (int p = 0; p < num_pts; p++)
    {
        for (int j = 0; j < NUM_GRID_PTS2; j++)
        {
            hold_value = (Vout[p * NUM_GRID_PTS2 + j] + VoutReverse[p * NUM_GRID_PTS2 + j]) / 2.0;
            Vout[p * NUM_GRID_PTS2 + j] = hold_value;
        }
    }
    ...
    if (GEOMETRY == 0)
    {
        initInjectIonSphere_101(NUM_DIV_QTH, NUM_DIV_VEL, TEMP_ELC, TEMP_ION, DRIFT_VEL_ION, MACH,
                                MASS_SINGLE_ION, BOLTZMANN, PI, d_QCOM.getDevPtr(),
                                d_VCOM.getDevPtr(), d_GCOM.getDevPtr(), debugMode, debugFile);
    }
    else if (GEOMETRY == 1)
    {
        initInjectIonCylinder_101(NUM_DIV_QTH, NUM_DIV_VEL, TIME_EVOL, RAD_CYL, HT_CYL, evolTe,
                                  evolTi, evolVz, evolMach, MASS_SINGLE_ION, BOLTZMANN, PI,
                                  d_QCOM.getDevPtr(), d_VCOM.getDevPtr(), d_GCOM.getDevPtr(),
                                  debugMode, debugFile);
    }
    ...
    *************************/
    ...
    checkIonDustBounds_101<<<blocksPerGridIon, DIM_BLOCK>>>(
        ...
        d_RAD_DUST_SQRD.getDevPtr(), d_NUM_DUST.getDevPtr(), d_posDust.getDevPtr());
    ...
    if (GEOMETRY == 0)
    {
        injectIonSphere_101<<<blocksPerGridIon, DIM_BLOCK>>>(
            ...
            randStates.getDevPtr(), d_RAD_SPH.getDevPtr(), d_boundsIon.getDevPtr(),
            d_GCOM.getDevPtr(), d_QCOM.getDevPtr(), d_VCOM.getDevPtr(), d_NUM_DIV_QTH.getDevPtr(),
            d_NUM_DIV_VEL.getDevPtr(), d_SOUND_SPEED.getDevPtr(), d_TEMP_ION.getDevPtr(),
            d_PI.getDevPtr(), d_TEMP_ELC.getDevPtr(), d_MACH.getDevPtr(),
            d_MASS_SINGLE_ION.getDevPtr(), d_BOLTZMANN.getDevPtr(), d_CHARGE_ION.getDevPtr(), xac);
    ...
    else if (GEOMETRY == 1)
    {
        ...
        injectIonCylinder_101<<<blocksPerGridIon, DIM_BLOCK>>>(
            ...
            d_velIon.getDevPtr(), // -->
            d_accIon.getDevPtr(), // -->
            randStates.getDevPtr(), d_RAD_CYL.getDevPtr(), d_HT_CYL.getDevPtr(),
            d_boundsIon.getDevPtr(), // <--
            d_GCOM.getDevPtr(), d_QCOM.getDevPtr(), d_VCOM.getDevPtr(), d_NUM_DIV_QTH.getDevPtr(),
            d_NUM_DIV_VEL.getDevPtr(), d_SOUND_SPEED.getDevPtr(), d_TEMP_ION.getDevPtr(),
            d_PI.getDevPtr(), d_TEMP_ELC.getDevPtr(), d_MACH.getDevPtr(),
            d_MASS_SINGLE_ION.getDevPtr(), d_BOLTZMANN.getDevPtr(), d_CHARGE_ION.getDevPtr(),
    ...
    kick_100<<<blocksPerGridIon, DIM_BLOCK>>>(d_velIon.getDevPtr(), // {{{
                                              d_accIon.getDevPtr(), d_HALF_TIME_STEP.getDevPtr());
    ...
    calcIonDustAcc_102<<<blocksPerGridIon, DIM_BLOCK>>>(
        ...
        d_posDust.getDevPtr(),    // <--
        d_NUM_ION.getDevPtr(), d_NUM_DUST.getDevPtr(), d_SOFT_RAD_SQRD.getDevPtr(),
        d_ION_DUST_ACC_MULT.getDevPtr(), d_minDistDust.getDevPtr());
    ...
    // NUM_TIME_STEP is the number of dust time steps
    {
        ...
        for (int j = 1; j <= N_IONDT_PER_DUSTDT; j++)
        {
            ...
            if (GEOMETRY == 0)
            {
                KDK_100<<<blocksPerGridIon, DIM_BLOCK>>>(
                    ...
                    d_accIonDust.getDevPtr(), // <--
                    d_boundsIon.getDevPtr(),  // <-->
                    d_minDistDust.getDevPtr(), d_M_FACTOR.getDevPtr(), d_ION_TIME_STEP.getDevPtr(),
                    GEOMETRY, d_RAD_SPH_SQRD.getDevPtr(), NULL, d_RAD_DUST.getDevPtr(),
                    d_NUM_DUST.getDevPtr(),
                    d_posDust.getDevPtr(), // <--
                    d_NUM_ION.getDevPtr(), d_SOFT_RAD_SQRD.getDevPtr(),
                    d_ION_DUST_ACC_MULT.getDevPtr(), d_RAD_COLL_MULT.getDevPtr());
            ...
            else if (GEOMETRY == 1)
            {
                KDK_100<<<blocksPerGridIon, DIM_BLOCK>>>(
                    ...
                    d_RAD_CYL_SQRD.getDevPtr(), d_HT_CYL.getDevPtr(), d_RAD_DUST.getDevPtr(),
                    d_NUM_DUST.getDevPtr(), d_posDust.getDevPtr(), d_NUM_ION.getDevPtr(),
                    d_SOFT_RAD_SQRD.getDevPtr(), d_ION_DUST_ACC_MULT.getDevPtr(),
            ...
            if (GEOMETRY == 0)
            {
                ...
                injectIonSphere_101<<<blocksPerGridIon, DIM_BLOCK>>>(
                    d_posIon.getDevPtr(), d_velIon.getDevPtr(), d_accIon.getDevPtr(),
                    randStates.getDevPtr(), d_RAD_SPH.getDevPtr(), d_boundsIon.getDevPtr(),
                    d_GCOM.getDevPtr(), d_QCOM.getDevPtr(), d_VCOM.getDevPtr(),
                    d_NUM_DIV_QTH.getDevPtr(), d_NUM_DIV_VEL.getDevPtr(), d_SOUND_SPEED.getDevPtr(),
                    d_TEMP_ION.getDevPtr(), d_PI.getDevPtr(), d_TEMP_ELC.getDevPtr(),
                    d_MACH.getDevPtr(), d_MASS_SINGLE_ION.getDevPtr(), d_BOLTZMANN.getDevPtr(),
                    d_CHARGE_ION.getDevPtr(), xac);
            ...
            if (GEOMETRY == 1)
            {
                ...
                injectIonCylinder_101<<<blocksPerGridIon, DIM_BLOCK>>>(
                    ...
                    d_TEMP_ION.getDevPtr(), d_PI.getDevPtr(), d_TEMP_ELC.getDevPtr(),
                    d_MACH.getDevPtr(), d_MASS_SINGLE_ION.getDevPtr(), d_BOLTZMANN.getDevPtr(),
                    d_CHARGE_ION.getDevPtr(), plasma_counter, counter_part,
                    xac); // <--
            ...
            calcDustIonAcc_103<<<blocksPerGridIon, DIM_BLOCK>>>(
                ...
                d_NUM_DUST.getDevPtr(), d_NUM_ION.getDevPtr(), d_INV_DEBYE.getDevPtr(),
            ...
            setCollisionList_105<<<blocksPerGridIon, DIM_BLOCK>>>(d_collList.getDevPtr(),
                                                                  set_value);
            ...
            ionCollisions_105<<<blocksPerGridIon, DIM_BLOCK>>>(
                d_collList.getDevPtr(), d_TEMP_GAS.getDevPtr(), d_MASS_SINGLE_ION.getDevPtr(),
                d_BOLTZMANN.getDevPtr(), d_I_CS_RANGES.getDevPtr(), d_TOT_ION_COLL_FREQ.getDevPtr(),
                d_SIGMA_I1.getDevPtr(), d_SIGMA_I2.getDevPtr(), d_SIGMA_I_TOT.getDevPtr(),
                d_velIon.getDevPtr(), randStates.getDevPtr(), d_collision_counter.getDevPtr());
            ...
            for (int c = 0; c < numCommands; c++)
            {
                ...
                else if (commands[c] == 4)
                {
                    ...
                    for (int g = 0; g < NUM_DUST; g++)
                    {
                        ...
                        elcCurrent =
                            ELC_CURRENT_0 * ION_TIME_STEP *
                            exp((-1.0) * CHARGE_ELC * dustPotential / (BOLTZMANN * TEMP_ELC));
            ...
            if (TIME_EVOL > 0)
            {
                ...
                if (j % N_IONDT_PER_PLASMADT == 0)
                {
                    ...
                    vs_sq = 8 * BOLTZMANN * TEMP_ION / PI / MASS_SINGLE_ION +
                            DRIFT_VEL_ION * DRIFT_VEL_ION;
                    RAD_COLL_MULT = 2 * Q_DIV_M * COULOMB_CONST / RAD_DUST / vs_sq;
                    ELC_CURRENT_0 = 4.0 * PI * RAD_DUST_SQRD * DEN_FAR_PLASMA * CHARGE_ELC *
                                    sqrt((BOLTZMANN * TEMP_ELC) / (2.0 * PI * ELC_MASS));
                    EXTERN_ELC_MULT = ((RAD_SPH / DEBYE) + 1.0) * exp(-RAD_SPH / DEBYE) *
                                      (CHARGE_SINGLE_ION * DEN_FAR_PLASMA * DEBYE) * (Q_DIV_M) /
                                      (PERM_FREE_SPACE);
            ...
            else if (commands[c] == 5)
            {
                ...
                for (int j = 0; j < NUM_DUST; j++)
                {
                    ...
                    for (int g = j + 1; g < NUM_DUST; g++)
                    {
                        ...
                        distSquared =
                            distdd.x * distdd.x + distdd.y * distdd.y + distdd.z * distdd.z;

                        // calculate the hard distance
                        dist = sqrt(distSquared);

                        // calculate a scalar intermediate
                        linForce = DUST_DUST_ACC_MULT * (simCharge[j]) * (simCharge[g]) /
                                   (dist * dist * dist);
                    ...
                    rhoDustsq = posDust[j].x * posDust[j].x + posDust[j].y * posDust[j].y;
                    ...
                    {
                        acc = OMEGA2_DIV_M * simCharge[j] * (rhoDust - radialConfine) / rhoDust;
                        accDust[j].x += acc * posDust[j].x;
                        accDust[j].y += acc * posDust[j].y;
                    }
                    ...
                    if (USE_GRAV == 1)
                    {
                        ...
                        ht2 = ht * ht;
                        acc = -8083 + 553373 * ht + 2.0e8 * ht2 - 3.017e10 * ht * ht2 +
                              1.471e12 * ht2 * ht2 - 2.306e13 * ht * ht2 * ht2;
        ...
        for (int j = 0; j < NUM_DUST; j++)
        {
            ionOnDustAccFile << accDustIon[j * NUM_ION].x / N_IONDT_PER_DUSTDT * MASS_DUST << ", "
                             ...
                             << momIonDust[j].z * mom_const * MASS_DUST / dust_dt * 2 << ", "
                             << std::endl;
        ...
        if (i % 10 == 0)
        { // print every 10 dust time steps
    ...
    ***********************/
    ...
    if (debugMode)
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            debugFile << "#: " << i << " X: " << posIon[i].x << " Y: " << posIon[i].y
        ...
        {
            int ID = NUM_ION - i;
            debugFile << "#: " << ID << " X: " << posIon[ID].x << " Y: " << posIon[ID].y
                      << " Z: " << posIon[ID].z << std::endl;
        }
        ...
        for (int i = 0; i < 20; i++)
        {
            debugFile << "#: " << i << " X: " << velIon[i].x << " Y: " << velIon[i].y
        ...
        {
            int ID = NUM_ION - i;
            debugFile << "#: " << ID << " X: " << velIon[ID].x << " Y: " << velIon[ID].y
                      << " Z: " << velIon[ID].z << std::endl;
        }
        ...
        for (int i = 0; i < 20; i++)
        {
            debugFile << "#: " << i << " X: " << accIon[i].x << " Y: " << accIon[i].y
                      << " Z: " << accIon[i].z << std::endl;
        }
        ...
        {
            int ID = NUM_ION - i;
            debugFile << "#: " << ID << " X: " << accIon[ID].x << " Y: " << accIon[ID].y
                      << " Z: " << accIon[ID].z << std::endl;
        }
