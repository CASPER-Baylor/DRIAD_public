...
int main(int argc, char *argv[])
{
	...
	************************/
	...
	const int NUM_ION = static_cast<int>(
	...
	if (debugMode)
	{
		...
		{
			debugFile << "X: " << posDust[i].x << " Y: " << posDust[i].y << " Z: " << posDust[i].z << " Q: " << posDust[i].w << " VX: " << velDust[i].x << " VY " << velDust[i].y << " VZ " << velDust[i].z << std::endl;
		}
	...
	if (debugMode)
	{
		...
		for (int i = 0; i < 10; i++)
		{
			debugFile << "X: " << posIon[i].x << " Y: " << posIon[i].y << " Z: " << posIon[i].z << std::endl;
		}
		...
		for (int i = 0; i < 10; i++)
		{
			debugFile << "X: " << velIon[i].x << " Y: " << velIon[i].y << " Z: " << velIon[i].z << std::endl;
		}
	...
	for (int p = 0; p < num_pts; p++)
	{
		...
		boundaryEField_101<<<blocksPerTable, DIM_BLOCK2, sizeof(float4) * DIM_BLOCK2>>>(d_GRID_POS.getDevPtr(),
	...
	// zero the ionDustAcc
	zeroDustIonAcc_103<<<blocksPerGridIon, DIM_BLOCK>>>(d_accDustIon.getDevPtr(),
	...
	*************************/
	...
	// NUM_TIME_STEP is the number of dust time steps
	{
		...
		for (int j = 1; j <= N_IONDT_PER_DUSTDT; j++)
		{
			...
			calcIonDensityPotential_102<<<blocksPerGridGrid, DIM_BLOCK, sizeof(float4) * DIM_BLOCK>>>(
				d_gridPos.getDevPtr(), // {{{
				d_posIon.getDevPtr(),
			...
			for (int c = 0; c < numCommands; c++)
			{
				...
				if (commands[c] == 1)
				{
					...
					if (isnan(posIon[ionTraceIndex].x) || isnan(posIon[ionTraceIndex].y) || isnan(posIon[ionTraceIndex].z))
					{
			...
			else if (commands[c] == 5)
			{
				...
				for (int j = 0; j < NUM_DUST; j++)
				{
					...
					for (int g = j + 1; g < NUM_DUST; g++)
					{
						...
						distSquared = distdd.x * distdd.x + distdd.y * distdd.y + distdd.z * distdd.z;

						// calculate the hard distance
						dist = sqrt(distSquared);

						// calculate a scalar intermediate
						linForce = DUST_DUST_ACC_MULT * (simCharge[j]) * (simCharge[g]) / (dist * dist * dist);
					...
					accDust[j].z -= q_div_m * E_FIELD * (4.0 * floor(FREQ * dust_time) - 2.0 * floor(2.0 * FREQ * dust_time) + 1.);
