...
template <class Type> class CUDAvar
{
 ...
 *
 */
template <class Type> CUDAvar<Type>::CUDAvar()
{
    hostPtr = NULL;
    devPtr = NULL;
    memSize = 0;
 ...
 *
 */
template <class Type> CUDAvar<Type>::CUDAvar(int newMemSize)
{
 ...
 *
 */
template <class Type> CUDAvar<Type>::CUDAvar(Type *newHostPtr, int newMemSize)
{
 ...
 *
 */
template <class Type> CUDAvar<Type>::~CUDAvar() { cudaFree(devPtr); }
 ...
 *
 */
template <class Type> void CUDAvar<Type>::setHostPtr(Type *newPtr)
{
 ...
 *
 */
template <class Type> void CUDAvar<Type>::allocMem(int newMemSize)
{
 ...
 *
 */
template <class Type> bool CUDAvar<Type>::checkHostPtr(Type *ptr)
{
    if (ptr == hostPtr)
    {
        return true;
 ...
 *
 */
template <class Type> void CUDAvar<Type>::hostToDev()
{

    if (memSize != 0 & hostPtr != NULL)
    {
        // copy the host value to the gpu
        cudaStatus = cudaMemcpy(devPtr, hostPtr, memSize, cudaMemcpyHostToDevice);
 ...
 *
 */
template <class Type> void CUDAvar<Type>::devToHost()
{

    if (memSize != 0 & hostPtr != NULL)
    {
        // copy the GPU value to the host
        cudaStatus = cudaMemcpy(hostPtr, devPtr, memSize, cudaMemcpyDeviceToHost);
        // check if the memory copy was successful
        if (cudaStatus != cudaSuccess)
        {
            // CUDAerr error;
 ...
 *
 */
template <class Type> bool CUDAvar<Type>::compare()
{

    Type *testVal = new Type[memSize / sizeof(Type)];

    // copy the GPU value to the host
    cudaStatus = cudaMemcpy(testVal, devPtr, memSize, cudaMemcpyDeviceToHost);
    // check if the memory copy was successful
    if (cudaStatus != cudaSuccess)
    {
        // CUDAerr error;
